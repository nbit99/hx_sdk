...
import (
	...
	"encoding/hex"
	"fmt"
	"github.com/nbit99/hx_sdk/common"
	"strconv"
	"strings"

	ierr "github.com/nbit99/hx_sdk/err"
	"math"
)
...
func dumpHXPrivateKey(seed []byte, net string, account, index int) (string, error) {
	key, err := ExportWif(seed, uint32(account), uint32(index))
	if err != nil {
...
//	wif, err := ExportWif(seed, uint32(req.Account), uint32(req.Index))
//	if err != nil {
...
func convertHXSymbol(s string) string {
	...
	assets = map[string]string{
		...
		CoinLTC:    CoinLTC,
		CoinPAX:    CoinERCPAX,
		CoinERCPAX: CoinERCPAX,
		CoinELF:    CoinERCELF,
		CoinERCELF: CoinERCELF,
		CoinBCH:    CoinBCH,
	}
...
func HxBind(ref, wif, chainId, originAddr, coinAddr, coinType, crossWif string) (buf []byte, err error) {
	if coinType == CoinBCH && strings.HasPrefix(originAddr, "bitcoincash:") == false {
		originAddr = "bitcoincash:" + originAddr
	}
	cwif := crossWif

	return BuildBindAccountTransaction(ref, wif, coinAddr, 0, originAddr, coinType, cwif, "", chainId)
}
func hxBind(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	if coinType == CoinBCH && strings.HasPrefix(origAddr, "bitcoincash:") == false {
		origAddr = "bitcoincash:" + origAddr
	}
	cwif := getStringField(formJson, fieldCrossWif)

	return BuildBindAccountTransaction(ref, wif, coinAddr, 0, origAddr, coinType, cwif, "", cid)
}
...
func HxUnbind(ref, wif, chainId, origAddr, coinAddr, coinType, crossWif string) (buf []byte, err error) {
	...
	return BuildUnBindAccountTransaction(ref, wif, coinAddr, 20000, origAddr, coinType, crossWif, chainId)
}
func hxUnbind(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	return BuildUnBindAccountTransaction(ref, wif, coinAddr, 20000, origAddr, coinType, cwif, cid)
}
...
func HxTransfer(ref, wif, chainId, fromAddr, toAddr, coinType string, transferAmount, fee string, memo string, guaranteeId string) (buf []byte, err error) {
	...
	return BuildTransferTransaction(ref, wif, fromAddr, toAddr, memo, assetId, tranAmt, tranFee, coinType, gid, chainId)
...
func hxTransfer(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	return BuildTransferTransaction(ref, wif, from, to, sbak, assetId, tranAmt, tranFee, coinType, gid, cid)
}
...
func HxWithdraw(ref, wif, chainId, fromAddr, toAddr, coinType, transferAmt, fee string) (buf []byte, err error) {
	...
	return BuildWithdrawCrosschainTransaction(ref, wif, fromAddr, tranFee, toAddr, coinType, assetId, transferAmt, chainId)
}
...
func hxWithdraw(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	return BuildWithdrawCrosschainTransaction(ref, wif, from, tranFee, to, coinType, assetId, tranAmt, cid)
}
...
func HxRegister(ref, wif, chainId, accountName, accountAddr, fee, guaranteeId string) (buf []byte, err error) {
	...
	pubkey, err := DerivePubkey(wif)
	if err != nil {
		return
	}

	return BuildRegisterAccountTransaction(ref, wif, accountAddr, pubkey, fee_value, guaranteeId, accountName, chainId)
}
...
func hxRegister(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	gid := getGuaranteeID(formJson)
	return BuildRegisterAccountTransaction(ref, wif, coinAddr, pubkey, fee, gid, name, cid)
}
...
func HxMining(ref, wif, chainId, coinType, accountId, addr, miningAmount, fee, minerId string) (buf []byte, err error) {
	...
	assetId := getAssetId(coinType)
	// _ = minerAddr
	return BuildLockBalanceTransaction(ref, wif, addr, accountId, assetId, tranAmt, feeValue, minerId, minerInvalidAddress, chainId)
}
...
func hxMining(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	// _ = minerAddr
	return BuildLockBalanceTransaction(ref, wif, addr, accountId, assetId, tranAmt, fee, minerID, minerInvalidAddress, cid)
}
...
func HxRewards(ref, wif, chainId, accountAddr string, payList []string, fee string, guaranteeId string) (buf []byte, err error) {
	...
	return BuildObtainPaybackTransaction(ref, wif, accountAddr, feeAmt, payList, guaranteeId, chainId)
}
...
func hxRewards(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	gid := getGuaranteeID(formJson)
	return BuildObtainPaybackTransaction(ref, wif, addr, tranFee, ps, gid, cid)
}
...
func HxForecloseBalance(ref, wif, chainId, accountAddr, accountId, assetId, minerId, amount, fee string) (buf []byte, err error) {
	...
	return BuildRedeemBalanceTransaction(ref, wif, accountAddr, accountId, assetId, tranAmt, tranFee, minerId, minerInvalidAddress, chainId)
}
...
func hxRedeem(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	return BuildRedeemBalanceTransaction(ref, wif, addr, accountID, assetId, tranAmt, tranFee, minerID, minerInvalidAddress, cid)
}
...
func HxContractInvoke(ref, wif, chainId, accountAddr, contractAddr, contractMethod, contractArgs, fee, gasPrice, gasLimit, guaranteeId string) (buf []byte, err error) {
	...
	return BuildContractInvokeTransaction(ref, wif, accountAddr, feeAmt, gasPriceAmt, gasLimitAmt, contractAddr, contractMethod, contractArgs, guaranteeId, chainId)
}
...
func hxContractInvoke(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	return BuildContractInvokeTransaction(ref, wif, addr, fee, gasPrice, gasLimit, contractId, contractAPI, contractArgs, gid, cid)
}
func HxTransferToContract(ref, wif, chainId, accountAddr, contractAddr, amount, assetId, memo, fee, gasPrice, gasLimit, guaranteeId string) (buf []byte, err error) {
	...
	return BuildContractTransferTransaction(ref, wif, accountAddr, feeAmt, amountAmt, assetId, gasPriceAmt, gasLimitAmt, contractAddr, memo, guaranteeId, chainId)
}
func hxContractTransfer(ref, wif, cid string, dataJson, formJson map[string]interface{}) (buf []byte, err error) {
	...
	return BuildContractTransferTransaction(ref, wif, addr, fee, amount, assetID, gasPrice, gasLimit, contractId, memo, gid, cid)
}
